...
Omdat alle afstanden in een grid als dat van ons altijd gelijk zijn aan 1 (of een andere constante), wordt de score van een aanliggende node altijd 1 hoger dan de hudige node.
In het geval van een aanliggende node die naast het startpunt ligt is de voorwaardelijke g-score dus gelijk aan 1.
Vervolgens wordt deze voorwaardelijke g-score vergeleken met de g-score (niet voowaardelijk) van de aanliggende node.
...
Op dat moment is begint net als bij het algoritme van Lee een backtrace, maar ditmaal op basis van de ``vorige''-verwijzing die in iedere node is opgeslagen.
